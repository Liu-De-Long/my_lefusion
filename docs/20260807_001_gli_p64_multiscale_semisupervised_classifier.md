...
- 监督多尺度残差 U-Net 的患者等权验证最佳 focus mIoU 为 `0.573000`，ET/RC IoU 为
  `0.510031/0.635969`；相对 exp009 C0 历史绝对最佳提升约 `0.0248`。
- mean-teacher 在 6772 个未标签 patch 上运行至 epoch 8 早停；最佳 focus mIoU 为
  `0.572729`，没有超过监督模型。伪标签覆盖稳定在约 `19%–20%`、平均置信度约
  `0.993–0.995`，说明失败不是无伪标签，而是高置信教师偏差被继续复制。
- 两阶段 mask 外非零率均为 `0`、union Dice 均为 `1`，但三项性能门禁均失败，因此冻结
  test 未运行。
- 后续不重复相同 mean-teacher 配方。下一阶段优先把只由 T1c 与总 mask 推导的显式几何
  通道接入空间网络，并针对小区域和边界设计监督；仍固定同一 1000 patch 与患者级 val。
